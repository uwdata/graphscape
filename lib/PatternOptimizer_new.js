  ...
  for (var l = 1; l <= inputArray.length; l++) {
    ...
    for (var i = 0; i < inputArray.length-l+1; i++) {
      ...
      if (appear.length > 1 && !overlap && rythmic ){
        ...
        var patternDistance = distance(newPattern, uniqTransitionSets);

        
        var RPcoverage = patternDistance * appear.length / inputDistance;
        ...
        if( !Optimized.find(function(rp){ return s(rp.pattern) === s(newPattern); }) ){
          ...
          newPattern.patternScore = score(newPattern.coverage, uniqTransitionSets, newPattern.appear, newPattern.pattern );
