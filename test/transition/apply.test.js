...
const {apply, applyEncodingEditOp, applyTransformEditOp, applyMarkEditOp } = require('../../src/transition/apply');
...
describe('transition.apply', function () {
  describe('applyMarkEditOp', function () {
    it('should apply a mark edit operation correctly.', function () {
      ...
      const iSpec = applyMarkEditOp(startVL, destinationVL, markEditOp)
  ...
  describe('applyTransformEditOp', function () {
    it('should apply SCALE and AGGREGATE edit operations correctly.', async function () {
      ...
      iSpec = applyTransformEditOp(iSpec, destinationVL, aggEditOp)
    ...
    it('should apply ADD_FILTER/REMOVE_FILTER/MODIFY_FILTER edit operations correctly.', function () {
      ...
      let iSpec = applyTransformEditOp(_startVL, _endVL, addFilterOp)
      expect(iSpec.transform.length).to.eq(3);
      let removeFilterOp = editOps.find(eo => eo.name === "REMOVE_FILTER")
      iSpec = applyTransformEditOp(iSpec, _endVL, removeFilterOp)
      ...
      iSpec = applyTransformEditOp(iSpec, _endVL, modifyFilterOp)
  ...
  describe('applyEncodingEditOp', function () {
    it('should apply a REMOVE_Channel edit operation correctly.', function () {
      ...
      let iSpec = applyEncodingEditOp(startVL, destinationVL, removeEditOp)

      expect(JSON.stringify(iSpec.encoding)).to.eq("{}")
      expect(() => {
        applyEncodingEditOp(iSpec, destinationVL, removeEditOp);
      }).to.throw();
    });
    ...
    it('should apply a ADD edit operation correctly.', function () {
      ...
      let iSpec = applyEncodingEditOp(startVL, endVL, addEditoOp)

      expect(iSpec.encoding.x.field).to.eq("A")
      expect(() => {
        applyEncodingEditOp(iSpec, endVL, addEditoOp);
      }).to.throw();
    });
    ...
    it('should apply a MOVE edit operation correctly.', function () {
      ...
      let iSpec = applyEncodingEditOp(startVL, endVL, moveEditoOp)

      expect(iSpec.encoding.x.field).to.eq("A")
      expect(iSpec.encoding.y).to.eq(undefined)
      expect(() => {
        applyEncodingEditOp(iSpec, endVL, moveEditoOp);
      }).to.throw();
    });
    ...
    it('should apply a MODIFY edit operation correctly.', function () {
      ...
      let iSpec = applyEncodingEditOp(startVL, endVL, modifyEditOp)
    ...
    it('should apply a SWAP edit operation correctly.', function () {
      ...
      let iSpec = applyEncodingEditOp(startVL, endVL, swapEditOp)
  ...
  describe('apply', function () {
    ...
    it('should return an error if the resulted spec is invalid.', async function () {
      let startVL = {
        "$schema": "https://vega.github.io/schema/vega-lite/v4.json",
        "data": {"url": "data/penguins.json"},
        "mark": "point",
